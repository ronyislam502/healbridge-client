...
const DoctorCard = ({ doctor, className }: DoctorCardProps) => {
  ...
  return (
    <Card className={cn(
      "bg-white dark:bg-slate-900 rounded-xl overflow-hidden border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-2xl transition-all duration-500 group flex flex-col h-full ring-0",
      className
    )}>
        ...
        <div className="space-y-2 mb-3 flex-1">
          <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-[11px] font-medium">
            <div className="w-7 h-7 rounded-lg bg-slate-50 dark:bg-slate-800 flex items-center justify-center shrink-0">
              <Icons.mapPin className="w-3.5 h-3.5 text-teal-500" />
            </div>
            <span className="line-clamp-1 text-black text-lg">{doctor.currentWorkingPlace}</span>
          </div>
        </div>
      </CardContent>
      ...
      <CardFooter className="border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 p-4 flex items-center justify-between">
        <div className="flex flex-col">
          <span className="text-black text-[8px] font-black uppercase tracking-widest mb-0.5 italic">Fee</span>
          <span className="text-xl font-black text-green-900 dark:text-white italic tracking-tighter">
